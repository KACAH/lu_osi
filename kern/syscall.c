...
sys_exofork(void)
{
    ...
    struct Env *env;
    int res = env_alloc(&env, curenv->env_id);
    if (res < 0) {
        return res;
    }
...
// Check if given virtual address is valid for system call
//
// Return 0 on success, < 0 on error.  Errors are:
//	-E_INVAL if va >= UTOP, or va is not page-aligned.
int check_sys_va(void *va) {
...
// Check permission flags for system calls
//
// perm -- PTE_U | PTE_P must be set, PTE_AVAIL | PTE_W may or may not be set,
//         but no other bits may be set.
//
// Return 0 if perm is valid. < 0 on error. Errors are:
//   -E_INVAL if perm is invalid
int check_sys_perm(int perm) {
    // check if PTE_U and PTE_P bits are present
	int req_bits = PTE_U|PTE_P;
	if ((perm & req_bits) != req_bits) {
        return -E_INVAL;
    }
    // check if no extra bits are present
    int denied_bits = ~(PTE_U|PTE_P|PTE_AVAIL|PTE_W);
    if ((perm & denied_bits) > 0) {
        return -E_INVAL;
    }
    return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// LAB 4: Your code here.
    // check source and destination virtual address
    int res = check_sys_va(srcva);
    if (res < 0) {
        return res;
    }
    res = check_sys_va(dstva);
    if (res < 0) {
        return res;
    }
